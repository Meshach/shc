		...
		while(have_rows('page_details')): the_row();
			...
			if(get_row_layout() == 'call_to_actions'):
				...
				echo '<div class="container pb30" id="member-packages">';
					...
					echo '<h2 class="text-center">Packages</h2>
					...
					<div class="angle"></div><!-- .angle -->';
						echo '<div class="column medium-4" data-equalizer-watch>
							<div class="member-package">';
								echo '<div class="member-content">
									<h3>Basic</h3>
									<p>Receive unlimited General Fitness and Pilates mat classes, as well as, 30-day advanced online registration.</p>

									<p>Classes include Yoga, Total Barre, Stretch and Roll, Experienced and ALL Pilates Mat, and Cardio Blast.).</p>
								</div><!-- .join-content -->';
								echo '<p class="price">$79<span>/month</span></p>';
						...
						echo '<div class="column medium-4" data-equalizer-watch>
							<div class="member-package">';
								echo '<div class="member-content">
									<h3>Power Group Training</h3>
									<p>Receive all the privileges of the Basics Membership plus choose from 25 weekly Group Training POWER and Short Burst Training and Spin workouts per week.</p>

									<p>Short Burst classes include TRX, Kettlebell, Power Core, and Cardio Tramp. Spin is offered daily. 60-day advanced online registration. Membership discounts include 20% off for seniors (65+) and students (ID Required).</p>
								</div><!-- .join-content -->';
								echo '<p class="price">$129<span>/month</span></p>';
							echo '</div><!-- .join-banner -->
						</div><!-- .column medium-6 large-3 -->';
						...
						echo '<div class="column medium-4" data-equalizer-watch>
							<div class="member-package">';
								echo '<div class="member-content">
									<h3>Power + Pilates</h3>
									<p>Provides all privileges of the Power GT Membership PLUS FOUR  Reformer classes each month.( a value up to $130), choose from 15 Reformer group workouts weekly. The special discounts and 60-day advanced online registration are also available to Power + Pilates members.</p>
								</div><!-- .join-content -->';
								echo '<p class="price">$159<span>/month</span></p>';
							echo '</div><!-- .join-banner -->
						</div><!-- .column medium-6 large-3 -->';
						...
						echo '<div class="'.(($cta_count==1)?"medium-centered ":"").'column medium-6 text'.$cta_text.'">
							<div class="cta region'.$cta_bg.' '.(($cta_content==1)?" cta-content ":"").'">
								...
								if($cta_link && $cta_link_text){
									...
									echo '<a href="'.$cta_link.'" class="button '.$cta_btn.'"'.(($cta_link_target)?" target=\"_blank\"":"").'>'.$cta_link_text.'</a>';
								}
							echo '</div><!-- .cta -->
						</div><!-- .column medium-6 -->';
					}
						...
						<div class="carousel">';
						    foreach( $class as $post):
								...
								echo '<div class="carousel-item medium-4">
									<a data-open="'.$post->post_name.'" alt="'.get_the_title().'">';
										if($t_img) {
									...
									<h3><a data-open="'.$post->post_name.'" alt="Read '.get_the_title().'">'.get_the_title().'<span class="fa fa-angle-right"></span></a></h3>';
